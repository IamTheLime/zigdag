...
pub fn build(b: *std.Build) void {
    ...
    const lib_install = b.addInstallArtifact(lib, .{
        .dest_dir = .{ .override = .{ .custom = "python-dist/openpricing" } },
    });
...
fn addCrossCompileStep(
    ...
    step_name: []const u8,
    target_str: []const u8,
    openpricing_mod: *std.Build.Module,
) void {
    ...
    const generated_mod = b.addModule(b.fmt("generated_nodes_{s}", .{target_str}), .{
        ...
        .imports = &.{
            .{ .name = "openpricing", .module = openpricing_mod },
        },
    });
    ...
    const lib_install = b.addInstallArtifact(lib, .{
        .dest_dir = .{ .override = .{ .custom = b.fmt("{s}/openpricing", .{pkg_base_dir}) } },
    });
