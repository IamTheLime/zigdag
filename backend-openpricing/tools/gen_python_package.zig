...
pub fn main() !void {
    ...
    const model_name_raw = if (root.object.get("name")) |n| n.string else "openpricing";
...
fn generateTypesFile(allocator: std.mem.Allocator, root: std.json.Value, output_dir: []const u8) !void {
    ...
    try writer.writeAll(
        \\"""
        \\Type definitions for OpenPricing.
        \\
...
fn generatePyprojectToml(allocator: std.mem.Allocator, name: []const u8, version: []const u8, base_dir: []const u8, _: []const u8) !void {
    ...
    try writer.print(
        ...
        \\name = "{s}"
        \\version = "{s}"
        \\description = "OpenPricing model - high-performance pricing engine"
        \\readme = "README.md"
        ...
        \\
        \\[project.urls]
        \\"Homepage" = "https://github.com/anomalyco/openpricing"
        \\
        \\[tool.setuptools]
        \\packages = ["{s}"]
        \\
...
fn generateEngineFile(allocator: std.mem.Allocator, output_dir: []const u8, lib_suffix: []const u8) !void {
    ...
    const lib_name = if (std.mem.eql(u8, lib_suffix, "dylib"))
        "libopenpricing.dylib"
    else
        "libopenpricing.so";

    try writer.print(
        \\"""
        \\OpenPricing Engine - Core FFI bindings.
        \\"""
        \\
        \\from __future__ import annotations
        \\
