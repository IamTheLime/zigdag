...
pub fn ComptimeExecutorFromNodes(comptime nodes: []const PricingNode) type {
    ...
    return struct {
        ...
        numeric_node_values: [node_count]f64 = [_]f64{0.0} ** node_count,
        string_node_values: [node_count]?[]const u8 = [_]?[]const u8{null} ** node_count,
        ...
        pub fn setInputNum(self: *Self, comptime node_id: []const u8, value: f64) !void {
        ...
        fn execute(self: *Self, comptime output_node_id: []const u8) ExecutorError!f64 {
            inline for (execution_order) |node_idx| {
                ...
                if (OutputType == f64) {
                    self.numeric_node_values[node_idx] = try self.evaluateNode(node);
                } else if (OutputType == []const u8) {
                    self.string_node_values[node_idx] = try self.evaluateNode(node);
                } else {
                    @compileError("Unsupported node output type");
                }
            }

            // Return the requested output node (must be numeric)
            const output_idx = comptime comptime_parser.getNodeIndex(nodes, output_node_id);
            return self.numeric_node_values[output_idx];
        }
            ...
            return switch (node.operation) {
                ...
                .conditional_value_input => |op| {
                    ...
                    const str_value = switch (input_node.operation) {
                        ...
                        .dynamic_input_str => self.string_node_values[input_idx],
                ...
                .add => |op| {
                    ...
                    return self.numeric_node_values[a_idx] + self.numeric_node_values[b_idx];
                },
                .subtract => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.left_input_node_id);
                    const b_idx = comptime comptime_parser.getNodeIndex(nodes, op.right_input_node_id);
                ...
                .power => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.left_input_node_id);
                    const b_idx = comptime comptime_parser.getNodeIndex(nodes, op.right_input_node_id);
                    return std.math.pow(f64, self.numeric_node_values[a_idx], self.numeric_node_values[b_idx]);
                },
                .modulo => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.left_input_node_id);
                    const b_idx = comptime comptime_parser.getNodeIndex(nodes, op.right_input_node_id);
                    return @mod(self.numeric_node_values[a_idx], self.numeric_node_values[b_idx]);
                },

                // Unary operations
                .negate => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return -self.numeric_node_values[a_idx];
                },
                .abs => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @abs(self.numeric_node_values[a_idx]);
                },
                .sqrt => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @sqrt(self.numeric_node_values[a_idx]);
                },
                .exp => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @exp(self.numeric_node_values[a_idx]);
                },
                .log => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @log(self.numeric_node_values[a_idx]);
                },
                .sin => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @sin(self.numeric_node_values[a_idx]);
                },
                .cos => |op| {
                    const a_idx = comptime comptime_parser.getNodeIndex(nodes, op.input_node_id);
                    return @cos(self.numeric_node_values[a_idx]);
                },
                .funnel => |op| {
                ...
                .max => |op| {
                    ...
                    var result = self.numeric_node_values[comptime comptime_parser.getNodeIndex(nodes, op.node_input_ids[0])];
                    inline for (op.node_input_ids[1..]) |input_id| {
                        const val = self.numeric_node_values[comptime comptime_parser.getNodeIndex(nodes, input_id)];
                ...
                // Clamp
                .clamp => |op| {
                    const val = self.numeric_node_values[comptime comptime_parser.getNodeIndex(nodes, op.value)];
                    const min_val = self.numeric_node_values[comptime comptime_parser.getNodeIndex(nodes, op.min)];
                    const max_val = self.numeric_node_values[comptime comptime_parser.getNodeIndex(nodes, op.max)];
